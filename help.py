...
def createHelp():
    ...
    parser.add_argument('-di', '--input directory',
                        dest='dir_input', help='Directory for input files')
    parser.add_argument('-i', '--input file', dest='file_in',
                        help='Input raw data file of manually-extracted tiller structure')
    parser.add_argument('-i2', '--input file2', dest='file_in2',
                        nargs='+', help='Input configure files (separated by space)')
    parser.add_argument('-o', '--output file',
                        dest='file_out', help='Output file')
    parser.add_argument('-ih', '--image height', dest='img_H',
                        type=int, help='Image height in pixels')
    parser.add_argument('-lineNA', '--cultivar line name', dest='lineNA',
                        type=str, help='Cultivar specific key word in image file name (N or Y)')
    parser.add_argument('-sp', '--spike', dest='withSpike', type=int,
                        default=1, help='Canopy with spike (1) or de-spiked (0)')
    parser.add_argument('-rID', '--run ID', dest='runID', type=int,
                        help='Remark of current run ID, used as file prefix and random seed.')
    parser.add_argument('-log', '--printInfo2logFile', dest='log', type=int,
                        default=1, help='Log printed info to log file (1) or not (0).')
    parser.add_argument('-ow', '--overWrite', dest='ow', type=int,
                        default=0, help='Overwrite the existing files (1) or not (0).')
    parser.add_argument('-dr', '--draw3D', dest='d3', type=int,  # draw canopy?
                        default=0, help='Draw 3D graph of single wheat (1), for canopy (2), for both (3), for none (0)')
    ...
    while line_i < dataIn_len:
        ...
        if line.startswith('Scale'):
            ...
            try:
                ...
                raise Exception(
                    'Input key points file format error! Scale cannot be determined. Line: %d' % (line_i+1))
            if scale <= 0:
                raise Exception(
                    'Scale setting error! scale<=0. Line: %d' % (line_i+1))
            line_i += 1
            line = dataIn[line_i]
        if line.startswith('<<'):
            if plant_info_str:
                # base of ear (or top-most leaf)
                stem_tip = KPs_imageCoor_list[-1][-1][0]
                # stem last point
                stem_extendTip = KPs_imageCoor_list[0][-1][-1]
                ...
                if spikeInternodeLenZero and len(ear_KP_list[0]) > 0:
                    diff_vec = np.array(
                        ear_KP_list[0][0]) - leaves_KP_list[-1][0]
                    ear_KP_list[0] = [ear_KP_list[0][i] -
                                      diff_vec for i in range(len(ear_KP_list[0]))]
                if ear_KP_list[0]:
                ...
                if leafStraighten == 1:
                    ...
                    for leaf_i in leaves_KP_list:
                        ...
                        len_leaf_i = [np.array(leaf_i[j])-leaf_i[j-1]
                                      for j in range(1, len(leaf_i))]
                        len_leaf_i = sum([np.linalg.norm(c)
                                         for c in len_leaf_i])
                        leaf_i_endP = leaf_i[0]+vec_leaf_i*len_leaf_i
                        leaves_KP_list_new.append(
                            [leaf_i[0], list(leaf_i_endP)])
                    leaves_KP_list = leaves_KP_list_new
                plant_KP_virtualCoor_dict[plant_info_str] = {
                    'stem': stem_KP_list, 'leaf': leaves_KP_list, 'ear': ear_KP_list}
                plant_info_str = ''
            if keyWord in line.split(' ')[0]:
                image_fn = line[2:]
                plant_info_list = image_fn.split('.')[0].split(' ')
                # plant_info_list[0]=2-N-plant1,...
                cultivarID, plantID = plant_info_list[0].split('plant')
                cultivarID = cultivarID[0:-1]
                # plant_info_list[1]=(1), (2), ...
                tillerID = plant_info_list[1][1:-1]
                try:
                    int(tillerID)
                except:
                    print('Image file name: ', image_fn,
                          '. Extracted tillerID: ', tillerID+'.')
                    raise Exception(
                        'ERROR: tillerID NOT a integer! Image file name error?')
        ...
        if data_line and plant_info_str:
            ...
            if data_type.startswith('Stem'):
                ...
                stem_coor_list = [[float(c[0]), img_H-float(c[1])+1]
                                  for c in stem_coor_list]
                ...
                stem_coor_new = np.matrix(
                    np.array(stem_coor_list)-stem_coor_list[0]).T
                stem_coor_new = [[z_direct[1], -z_direct[0]],
                                 [z_direct[0], z_direct[1]]]*stem_coor_new/scale
            ...
            if data_type.startswith('Leaf'):
                ...
                leaf_coor_list = [[float(c[0]), img_H-float(c[1])+1]
                                  for c in leaf_coor_list]
                KPs_imageCoor_list.append([leaf_coor_list])
                num_leaf_KP = len(leaf_coor_list)
                leaf_coor_new = np.matrix(
                    np.array(leaf_coor_list)-stem_coor_list[0]).T
                leaf_coor_new = [[z_direct[1], -z_direct[0]],
                                 [z_direct[0], z_direct[1]]]*leaf_coor_new/scale
            ...
            if data_type.startswith('Panicle'):
                ...
                ear_coor_list = [[float(c[0]), img_H-float(c[1])+1]
                                 for c in ear_coor_list]
                KPs_imageCoor_list.append([ear_coor_list])
                num_ear_KP = len(ear_coor_list)
                ear_coor_new = np.matrix(
                    np.array(ear_coor_list)-stem_coor_list[0]).T
                ear_coor_new = [[z_direct[1], -z_direct[0]],
                                [z_direct[0], z_direct[1]]]*ear_coor_new/scale
...
def buildStem(barcode, LNC_Kt_Kr, organ_base, organ_tip, organ_diameter):
    ...
    y_in_order = [0, math.sqrt(3)*1/2, math.sqrt(3)
                  * 1/2, 0, -math.sqrt(3)*1/2, -math.sqrt(3)*1/2, 0]
    ...
    for pt_i in range(seg_number):
        ...
        dem0 = organ_diameter[0]*(organ_len-seg_len*pt_i) / \
            organ_len+organ_diameter[1]*seg_len*pt_i/organ_len
        radi0 = dem0/2
        x1 = organ_tip_temp[0]
        y1 = organ_tip_temp[1]
        z1 = organ_tip_temp[2]
        dem1 = organ_diameter[0]*(organ_len-seg_len*(pt_i+1)) / \
            organ_len+organ_diameter[1]*seg_len*(pt_i+1)/organ_len
        radi1 = dem1/2
        for i in range(6):
            p_s_01 = [radi0*x_in_order[i] + x0, radi0*y_in_order[i] + y0, z0]
            p_s_02 = [radi0*x_in_order[i+1] + x0,
                      radi0*y_in_order[i+1] + y0, z0]
            p_s_11 = [radi1*x_in_order[i] + x1, radi1*y_in_order[i] + y1, z1]
            p_s_12 = [radi1*x_in_order[i+1] + x1,
                      radi1*y_in_order[i+1] + y1, z1]
            M_stem = [barcode + p_s_01 + p_s_02 + p_s_11 + LNC_Kt_Kr] + \
                [barcode + p_s_02 + p_s_11 + p_s_12 + LNC_Kt_Kr]
            # print(M_stem)
    ...
    if abs(sin_a) > 0.001:
        ...
        b = rotation_vec[1]
        c = rotation_vec[2]
        row_1 = [a**2+(1-a**2)*cos_a, a*b*(1-cos_a) +
                 c*sin_a, a*c*(1-cos_a)-b*sin_a]
        row_2 = [a*b*(1-cos_a)-c*sin_a, b**2+(1-b**2)
                 * cos_a, b*c*(1-cos_a)+a*sin_a]
        row_3 = [a*c*(1-cos_a)+b*sin_a, c*b*(1-cos_a) -
                 a*sin_a, c**2+(1-c**2)*cos_a]
...
# z axis stem:[stem_seg_base stem_seg_tip]
def buildLeaf(barcode, LNC_Kt_Kr, organ_PT_coords, stem_seg_base, stem_seg_tip, widLen_relation_coef, leafWidFun, rollRad, sheathRollRad, transLen, ptDense, halfSplit, randZRot_max, tortAng, tortAng_start_pos, tortAng_end_pos, leafAng_coef):
    ...
    if tortAng_num != len(tortAng_start_pos) or tortAng_num != len(tortAng_end_pos):
        raise Exception(
            'Inconsistency of tortAng and their begin_pos and end_pos number!')
    ...
    leaf_segments_len = [np.linalg.norm(
        organ_PT_coords[i+1]-organ_PT_coords[i]) for i in range(leaf_segments_num)]
    leaf_len_max = sum(leaf_segments_len)
    # leafWidPattern
    leafWidMax = widLen_relation_coef[0]*leaf_len_max**2 + \
        widLen_relation_coef[1]*leaf_len_max + widLen_relation_coef[2]
    ...
    organ_PT_coords_new = np.dot(
        organ_PT_coords-stem_seg_base, coord_transform_mat.T)
    ...
    if abs(leafAng_coef-1) > 0.001:
        ...
        z_rot_mat = np.array(
            [[cos_delta, -sin_delta, 0], [sin_delta, cos_delta, 0], [0, 0, 1]])
        organ_PT_coords_new = np.dot(
            organ_PT_coords_new-organ_PT_coords_new[0], z_rot_mat)+organ_PT_coords_new[0]
    organ_PT_coords_new_dense = [
        organ_PT_coords_new[0].tolist()]  # leaf vein dense pt list
    ...
    rotation_matrix = np.array([[math.cos(randZRot), 0, math.sin(randZRot)], [
                               0, 1, 0], [-math.sin(randZRot), 0, math.cos(randZRot)]])
    ...
    tortAng_old = 0
    tot_leafPts = len(t)
    row = np.array([0.0]*(2*halfSplit+1)*3 *
                   tot_leafPts).reshape((2*halfSplit+1, 3, tot_leafPts))
    # pts setting: base -> tip
    transLen = min(max(0, transLen), leaf_len_max)
    for pt_i in range(tot_leafPts):
        if pt_i == tot_leafPts-1:
            x_axis_new2 = np.array(
                organ_PT_coords_new_dense[pt_i])-organ_PT_coords_new_dense[pt_i-1]
            # print('***',organ_PT_coords_new_dense[pt_i],organ_PT_coords_new_dense[pt_i-1])
        else:
            x_axis_new2 = np.array(
                organ_PT_coords_new_dense[pt_i+1])-organ_PT_coords_new_dense[pt_i]
        ...
        if pt_i < transLen//ptDense:
            R1 = sheathRollRad*(transLen/ptDense-pt_i) / \
                (transLen/ptDense)+rollRad_y*pt_i/(transLen/ptDense)
        else:
            R1 = rollRad_y
        alpha = math.pi/2
        ...
        for pos_i in range(len(tortAng_start_pos)):
            ...
            if tortAng_start_pos[pos_i] <= t[pt_i]/leaf_len_max and tortAng_end_pos[pos_i] >= t[pt_i]/leaf_len_max:
                tortAng_now = sum(tortAng[0:pos_i])+tortAng[pos_i]*(t[pt_i]/leaf_len_max-tortAng_start_pos[pos_i])/(
                    tortAng_end_pos[pos_i]-tortAng_start_pos[pos_i])  # leaf face direction alteration radian along leaf length
                break
        tortAng_old = tortAng_now
        # leaf twist matrix
        r_mat = np.array([[1, 0, 0], [0, math.cos(
            tortAng_now), -math.sin(tortAng_now)], [0, math.sin(tortAng_now), math.cos(tortAng_now)]])
        pt_coord_list = []
        for j in range(2*halfSplit+1):
            alpha_new = alpha-p[pt_i]*(halfSplit-j)/halfSplit/R1
            # print(alpha,p[pt_i],halfSplit,R1)
            temp_coord = [0, R1]+R1 * \
                np.array([-math.cos(alpha_new), -math.sin(alpha_new)])
            pt_coord = [0, temp_coord[1], temp_coord[0]]
            pt_coord = np.dot(pt_coord, r_mat)  # leaf twist
            pt_coord = np.dot(pt_coord, np.array(
                [x_axis_new2, y_axis_new2, z_axis_new2])) + organ_PT_coords_new_dense[pt_i]
            row[j, :, pt_i] = pt_coord
    M_leaf = np.zeros([0, 9])
    ...
    for i in range(1, len(t)):
        ...
        for j in range(1, 2*halfSplit+1):
            # tri_left_up
            pt_tri[2*j-2, :] = np.hstack((row[j-1, 0:3, i-1],
                                         row[j-1, 0:3, i], row[j, 0:3, i-1]))
            # tri_right_up
            pt_tri[2*j-1, :] = np.hstack((row[j-1, 0:3, i],
                                         row[j, 0:3, i], row[j, 0:3, i-1]))
...
def buildSpikeletCover(barcode, LNC_Kt_Kr, awnLen, awnThick, grainLen, grainThick1, grainThick2, awnAng, allowedLayerNum):
    ...
    if allowedLayerNum == 2:
        ...
        raise Exception(
            'Illegal allowed layer number for building spikelet cover!')
    for layer in range(allowedLayerNum):
        ...
        for i in range(3):
            if layer != allowedLayerNum-1:  # layer!=0 and allowedLayerNum-1
                p_s_01 = [radi_x0*x_in_order[i] + x0,
                          radi_y0*y_in_order[i] + y0, z0]
                p_s_02 = [radi_x0*x_in_order[i+1] + x0,
                          radi_y0*y_in_order[i+1] + y0, z0]
                p_s_11 = [radi_x1*x_in_order[i] + x1,
                          radi_y1*y_in_order[i] + y1, z1]
                p_s_12 = [radi_x1*x_in_order[i+1] + x1,
                          radi_y1*y_in_order[i+1] + y1, z1]
                M_patch = [barcode + p_s_01 + p_s_02 + p_s_11 + LNC_Kt_Kr[0:3]
                           ] + [barcode + p_s_02 + p_s_11 + p_s_12 + LNC_Kt_Kr[0:3]]
            else:  # last layer
                p_s_01 = [radi_x0*x_in_order[i] + x0,
                          radi_y0*y_in_order[i] + y0, z0]
                p_s_02 = [radi_x0*x_in_order[i+1] + x0,
    ...
    if awnLen > 0:
        ...
        # +math.sin(awnAng_abs0)/awnAng_abs0*(random.random()-0.5)*2*random_extent0 #*(1+(random.random()-0.5)*2*random_extent)
        awnAng_new[0] = -awnAng[0]
        # +math.sin(awnAng_abs1)/awnAng_abs1*(random.random()-0.5)*2*random_extent0 #*(1+(random.random()-0.5)*2*random_extent)
        awnAng_new[1] = -awnAng[1]
        # awnAng shrink to z axis in front view of a spikelet
        cos_yRotAng = math.cos(awnAng_new[0])
        sin_yRotAng = math.sin(awnAng_new[0])
        y_rot_mat = np.array([[cos_yRotAng, 0, sin_yRotAng], [
                             0, 1, 0], [-sin_yRotAng, 0, cos_yRotAng]])
        # awnAng shrink to z axis in side view of a spikelet
        cos_xRotAng = math.cos(awnAng_new[1])
        sin_xRotAng = math.sin(awnAng_new[1])
        x_rot_mat = np.array(
            [[1, 0, 0], [0, cos_xRotAng, -sin_xRotAng], [0, sin_xRotAng, cos_xRotAng]])
        ...
        z_rot_mat = np.array(
            [[cos_zRotAng, -sin_zRotAng, 0], [sin_zRotAng, cos_zRotAng, 0], [0, 0, 1]])
        x2 = 0
        y2 = 0
        z2 = awnLen+grainLen*0.05
        [x2, y2, z2] = np.dot(
            np.dot(np.dot([x2, y2, z2], y_rot_mat), x_rot_mat), z_rot_mat)+[x0, y0, z0]
        [x1, y1, z1] = (np.array([x2, y2, z2])+[x0, y0, z0])/2
        for i in range(3):
            p_s_01 = [radi_x[0]*x_in_order2[i] + x0,
                      radi_y[0]*y_in_order2[i] + y0, z0]
            p_s_02 = [radi_x[0]*x_in_order2[i+1] + x0,
                      radi_y[0]*y_in_order2[i+1] + y0, z0]
            p_s_11 = [radi_x[1]*x_in_order2[i] + x1,
                      radi_y[1]*y_in_order2[i] + y1, z1]
            p_s_12 = [radi_x[1]*x_in_order2[i+1] + x1,
                      radi_y[1]*y_in_order2[i+1] + y1, z1]
            M_patch = [barcode_new + p_s_01 + p_s_02 + p_s_11 + LNC_Kt_Kr[3:]
                       ]+[barcode_new + p_s_02 + p_s_11 + p_s_12 + LNC_Kt_Kr[3:]]
            M_spikelet_cover = np.vstack((M_spikelet_cover, M_patch))
        for i in range(3):
            p_s_01 = [radi_x[1]*x_in_order2[i] + x1,
                      radi_y[1]*y_in_order2[i] + y1, z1]
            p_s_02 = [radi_x[1]*x_in_order2[i+1] + x1,
                      radi_y[1]*y_in_order2[i+1] + y1, z1]
            p_s_11 = [radi_x[2]*x_in_order2[i] + x2,
                      radi_y[2]*y_in_order2[i] + y2, z2]
            p_s_12 = [radi_x[2]*x_in_order2[i+1] + x2,
                      radi_y[2]*y_in_order2[i+1] + y2, z2]
            M_patch = [barcode_new + p_s_01 + p_s_02 + p_s_11 + LNC_Kt_Kr[3:]
                       ]+[barcode_new + p_s_02 + p_s_11 + p_s_12 + LNC_Kt_Kr[3:]]
            M_spikelet_cover = np.vstack((M_spikelet_cover, M_patch))
...
def buildSpikelet(barcode, LNC_Kt_Kr, spikeletBranchLen, spikeletBranchThick, awnLen_lemma, awnThick, grainLen, grainWid, grainThick, grainLen2, grainWid2, grainThick2, awnAng_coef, glumeAng, allowedLayerNum, glumeAwnLen, spikeletAng, LEMMA_ANG):
    ...
    for i in range(6):
        ...
        M_spikeletBranch = [barcode_new + p_s_01 + p_s_02 + p_s_11 + LNC_Kt_Kr[0:3]] + [
            barcode_new + p_s_02 + p_s_11 + p_s_12 + LNC_Kt_Kr[0:3]]
...
#     sin_a_lemma1=math.sqrt(1-cos_a_lemma1**2)
    ...
    if LEMMA_ANG == -100:
        lemmaAng = 1 * \
            math.atan(grainWid/2/math.sqrt(centerSpikeletH **
                      2+grainLen*centerSpikeletH))
    else:
        lemmaAng = LEMMA_ANG
...
#     M_spikelet_all = np.vstack((M_spikelet_all,M_glume2))
    ...
    M_lemma1 = buildSpikeletCover(barcode_new, LNC_Kt_Kr, awnLen_lemma, awnThick_lemma,
                                  grainLen, grainWid, grainThick, awnAng_lemma, allowedLayerNum)
    y_rotate1 = np.array([[cos_a_lemma1, 0, sin_a_lemma1], [
                         0, 1, 0], [-sin_a_lemma1, 0, cos_a_lemma1]])
    ...
    # M_lemma3=M_lemma
    M_lemma3 = buildSpikeletCover(barcode_new, LNC_Kt_Kr, awnLen_lemma, awnThick_lemma,
                                  grainLen, grainWid, grainThick, awnAng_lemma, allowedLayerNum)
    ...
    awnAng_lemma2 = [0, 0]
    # lemma2Ang*(1+(random.random()-0.5)*2*0.05)
    awnAng_lemma2[0] = lemma2Ang*awnAng_coef[0]
    awnAng_lemma2[1] = spikeletAng*awnAng_coef[1]
    M_lemma2 = buildSpikeletCover(barcode_new, LNC_Kt_Kr, awnLen_lemma, awnThick_lemma,
                                  grainLen2, grainWid2, grainThick2, awnAng_lemma2, allowedLayerNum)
    triangle_num = np.size(M_lemma2, 0)
    off_set = np.tile(baseCoord, (triangle_num, 1))
    y_rotate2 = np.array([[cos_a_lemma2, 0, sin_a_lemma2], [
                         0, 1, 0], [-sin_a_lemma2, 0, cos_a_lemma2]])
    ...
    M_palea = buildSpikeletCover(barcode_new, LNC_Kt_Kr, awnLen_palea, awnThick_palea,
                                 grainLen, grainWid, grainThick, awnAng_palea, allowedLayerNum)
    triangle_num = np.size(M_palea, 0)
    y_rotate3 = np.array([[cos_a_palea1, 0, -sin_a_palea1],
                         [0, 1, 0], [sin_a_palea1, 0, cos_a_palea1]])
    ...
    y_rotate2 = np.array([[cos_a_lemma2, 0, sin_a_lemma2], [
                         0, 1, 0], [-sin_a_lemma2, 0, cos_a_lemma2]])
    M_palea2 = buildSpikeletCover(barcode_new, LNC_Kt_Kr, awnLen_palea, awnThick_palea,
                                  grainLen2, grainWid2, grainThick2, awnAng_palea, allowedLayerNum)
...
def buildEar(barcode, LNC_Kt_Kr, organ_PT_coords, spikeletNum, spikeletBranchLen, spikeletBranchThick, awnLen_list, awnThick, grainLen, grainWid, grainThick, grainLen2, grainWid2, grainThick2, grainShrinkCoef, awnAng_coef_bot, awnAng_coef_mid, awnAng_coef_top, glumeAng, lemmaAng, SPIKELET_ANG, zRotAng, earRachis_diameter, allowedLayerNum, glumeAwnLen):
    ...
    for pt_i in range(1, ear_segments_num):
        ...
        M_ear = buildStem(barcode_new, LNC_Kt_Kr[0:3], organ_base, organ_tip, [
                          earRachis_diameter, earRachis_diameter])  # plant, tiller, Position, organCode
    ...
    organ_len = np.linalg.norm(organ_vec)
    organ_direction = (organ_vec)/organ_len
    organ_PT_coords_new[pt_i] = organ_base + \
        organ_direction*(organ_len-grainShrinkCoef*grainLen)
    organ_tip = organ_PT_coords_new[pt_i]
    M_ear = buildStem(barcode_new, LNC_Kt_Kr[0:3], organ_base, organ_tip, [
                      earRachis_diameter, earRachis_diameter*0.3])  # plant, tiller, Position, organCode
    M_ear_all = np.vstack((M_ear_all, M_ear))
    # ear segments and length
    ear_segments_len = [np.linalg.norm(
        organ_PT_coords_new[i+1]-organ_PT_coords_new[i]) for i in range(ear_segments_num)]
    ear_len_max = sum(ear_segments_len)
    spikeletDist = ear_len_max/spikeletNum
    # 3D reconstruction of spikelets
    z_rotate_other = np.array([[math.cos(zRotAng), -math.sin(zRotAng), 0],
                              [math.sin(zRotAng), math.cos(zRotAng), 0], [0, 0, 1]])
    zRotAng = zRotAng+math.pi/2
    z_rotate_tip = np.array([[math.cos(zRotAng), -math.sin(zRotAng), 0],
                            [math.sin(zRotAng), math.cos(zRotAng), 0], [0, 0, 1]])
    ...
    for sp_i in range(1, spikeletNum+1):
        ...
        if sp_i_rank < 1/3:
            ...
            awnAng_coef_i[0] = awnAng_coef_bot[0] * \
                (1-sp_i_rank*3)+awnAng_coef_mid[0]*sp_i_rank*3
            awnAng_coef_i[1] = awnAng_coef_bot[1] * \
                (1-sp_i_rank*3)+awnAng_coef_mid[1]*sp_i_rank*3
        elif sp_i_rank < 2/3:
            awnLen = awnLen_list[1]
            awnAng_coef_i = awnAng_coef_mid
        else:
            awnLen = awnLen_list[2]
            awnAng_coef_i[0] = awnAng_coef_mid[0] * \
                (3-sp_i_rank*3)+awnAng_coef_top[0]*(sp_i_rank*3-2)
            awnAng_coef_i[1] = awnAng_coef_mid[1] * \
                (3-sp_i_rank*3)+awnAng_coef_top[1]*(sp_i_rank*3-2)
        ...
        if SPIKELET_ANG == -100:
            spikeletAng = (-1)**sp_i*0.8*math.atan(grainThick_i/2/math.sqrt(spikeletBranchLen **
                                                                            2+grainLen_i*spikeletBranchLen))  # angle of spikelets to spike rachis
        else:
        ...
        for seg_i in range(ear_segments_num):
            ...
            if dist_i <= seg_len_tot+0.00001:
                ...
                Origin_coord = organ_PT_coords_new[seg_i] + \
                    (dist_i-sum(ear_segments_len[0:seg_i]))*z_axis_new
                break
        y_axis_new = [0, 1, 0]
        ...
        M_spikelet = buildSpikelet(barcode, LNC_Kt_Kr[3:], spikeletBranchLen, spikeletBranchThick, awnLen, awnThick, grainLen_i, grainWid_i,
                                   grainThick_i, grainLen2_i, grainWid2_i, grainThick2_i, awnAng_coef_i, glumeAng, allowedLayerNum, glumeAwnLen, spikeletAng, lemmaAng)
        # spikeletAng (x rotate)
        x_rotate = np.array([[1, 0, 0], [0, math.cos(
            spikeletAng), -math.sin(spikeletAng)], [0, math.sin(spikeletAng), math.cos(spikeletAng)]])
        ...
        M_spikelet[:, 5:8] = off_set + \
            np.dot(M_spikelet[:, 5:8], coord_transform_mat)
        M_spikelet[:, 8:11] = off_set + \
            np.dot(M_spikelet[:, 8:11], coord_transform_mat)
        M_spikelet[:, 11:14] = off_set + \
            np.dot(M_spikelet[:, 11:14], coord_transform_mat)
        M_ear_all = np.vstack((M_ear_all, M_spikelet))
    return M_ear_all


def buildCanopy(M_multi_tillers, row_dist, sowing_width, plant_density, rowNum_simulate, rowLen_simulate, tillerAng, Kr_soil, groundH_mean, groundH_sd):
    ...
    M_canopy = np.zeros([plantNum_allRow*maxTillerSize +
                        round(row_dist*(rowNum_simulate-1)*2*rowLen_simulate), 17])
    print(np.size(M_canopy), np.size(M_canopy, 1), np.size(M_canopy, 0))
    tillerNum = len(M_multi_tillers)
    print('  In all ' + str(tillerNum) + ' tillers collected, building canopy with %d plants (rowLen:%dcm, rowNum:%d):' %
          (plantNum_allRow, rowLen_simulate, rowNum_simulate))
    start_pos = 0
    for row_i in range(rowNum_simulate):
        ...
        for plant_i in range(plantsNum_oneRow):
            ...
            # LEO-START
            ground_height = random.gauss(groundH_mean, groundH_sd) 
            off_set += [0, 0, ground_height]   
            # LEO-END
            
            M_tiller = np.zeros([triangle_num, 17])
            M_tiller[:, 0:5] = M_multi_tillers[tiller_i][:, 0:5]
            M_tiller[:, 14:] = M_multi_tillers[tiller_i][:, 14:]
            M_tiller[:, 5:8] = off_set + \
                np.dot(M_multi_tillers[tiller_i][:, 5:8], r_mat)
            M_tiller[:, 8:11] = off_set + \
                np.dot(M_multi_tillers[tiller_i][:, 8:11], r_mat)
            M_tiller[:, 11:14] = off_set + \
                np.dot(M_multi_tillers[tiller_i][:, 11:14], r_mat)
    ...
    corner_0_record = False
    for x_i in np.linspace(xmin, xmax, round(xmax-xmin)+1)[0:-1]:
        for y_i in np.linspace(ymin, ymax, round(ymax-ymin)+1)[0:-1]:
            if not corner_0_record:
                print("***********corner 1: ", (x_i, y_i))
                corner_0_record = True  # tobecontinued

            p1 = [x_i, y_i, zmin]
            p2 = [x_i, y_i+1, zmin]
            p3 = [x_i+1, y_i, zmin]
            p4 = [x_i+1, y_i+1, zmin]
            M_soil = np.array([[0, 0, -6, 1, 0] + p1 + p2 + p4 + [0, 0, Kr_soil]] + [
                              [0, 0, -6, 1, 0] + p1 + p3 + p4 + [0, 0, Kr_soil]])
            M_canopy[current_pos:current_pos+2, :] = M_soil
            current_pos += 2
    print("***********corner 2: ", (x_i, y_i))
...
def TillerSkeletonPlot(plant_KP_virtualCoor_dict, figOutPrefix, showOn):
    ...
    # stem_PTs_num-1: do not use the last point (extendTip)
    for pt_i in range(1, stem_PTs_num-1):
        plt.plot([stem_PTs[pt_i-1][0], stem_PTs[pt_i][0]], [stem_PTs[pt_i-1][1], stem_PTs[pt_i]
                 [1]], color='k', marker='*', linestyle='-', linewidth=5, markersize=6)
    ...
    for leaf_i in range(num_leaf):
        ...
        for pt_i in range(1, leaf_PTs_num):
            plt.plot([leaf_PTs[pt_i-1][0], leaf_PTs[pt_i][0]], [leaf_PTs[pt_i-1][1], leaf_PTs[pt_i]
                     [1]], color='g', marker='o', linestyle='-', linewidth=3, markersize=6)
    ear_PTs = plant_KP_virtualCoor_dict['ear'][0]
    ear_PTs_num = len(ear_PTs)
    # stem_PTs_num-1: do not use the last point (extendTip)
    for pt_i in range(1, ear_PTs_num):
        plt.plot([ear_PTs[pt_i-1][0], ear_PTs[pt_i][0]], [ear_PTs[pt_i-1][1], ear_PTs[pt_i]
                 [1]], color='y', marker='s', linestyle='-', linewidth=8, markersize=6)
...
def awnAngCoef_Fitter(glumeAng, spikeletAng, awnAng1, awnAng2):
    awnAngCoef1 = 0
    awnAngCoef2 = 0
    fit_error = fit_residuals(
        [awnAngCoef1, awnAngCoef2], glumeAng, spikeletAng, awnAng1, awnAng2)
    itera_test = 0
    while itera_test < 50:
        ...
        paras0 = np.array([awnAngCoef1_ini, awnAngCoef2_ini]
                          )  # theta,Amax,alpha,Rd
        popt = fmin_slsqp(fit_residuals, paras0, bounds=[
                          (-1, 1), (-1, 1)], args=(glumeAng, spikeletAng, awnAng1, awnAng2), iprint=0)
        popt = popt[0:]
        fit_error_new = fit_residuals(
            popt[0:], glumeAng, spikeletAng, awnAng1, awnAng2)
        if fit_error_new < fit_error:
            fit_error = fit_error_new
            awnAngCoef1, awnAngCoef2 = popt
        itera_test = itera_test+1
    print('            Awn angle fitting done! awnAngCoef: %.2f, %.2f. Fit_error: %.3f' % (
        awnAngCoef1, awnAngCoef2, fit_error))
    return popt
...
if __name__ == "__main__":
    ...
    log2file = op.log
    if log2file:
        sys.stdout = Logger(os.path.abspath(
            Path(__file__).parent)+'\log_wheatCanopyConstructor2')
    width_format = 94
    ...
    print("fnOut_str : ", fnOut_str)  # debug
    overWrite = op.ow
    fnOut_list = fnOut_str.split('\\')
    resultDir = Path(fnOut_str).parent  # '\\'.join(fnOut_list[0:-1])
    ...
    data = json.dumps(vars(op), indent=4, ensure_ascii=False,
    ...
    for line in open(config_morphology_fn, 'r'):
        ...
        if words[0] == 'plant_density':
            ...
            # rows, rows simulated for a canopy
            rowNum_simulate = int(words[1])
        elif words[0] == 'rowLen':
            # cm, length of one row simulated for a canopy
            rowLen_simulate = float(words[1])
        elif words[0] == 'row_dist':
            # 20 # cm, distance between two growing rows
            row_dist = float(words[1])
        elif words[0] == 'sowing_width':
            # 10 # cm, the width of wheat seeds spread in one row
            sowing_width = float(words[1])
        ...
        # LEO - START
        elif words[0] == 'groundH_mean':
            groundH_mean = float(words[1])  # ground height mean
        elif words[0] == 'groundH_sd':
            groundH_sd = float(words[1])
        # LEO - END

        elif words[0] == 'spikeInternodeLenZero':
            # set spike Internode Length to 0
            spikeInternodeLenZero = float(words[1])
            ...
            # Detailed layers for building glume, lemma and grain. Can only be 2,3 or 4.
            ...
            # cm, cut leaf length direction into pieces with a size of ptDense
            ptDense = float(words[1])
        elif words[0] == 'halfSplit':
            # cut leaf width direction into halfSplit*2 pieces
            halfSplit = int(words[1])
        elif words[0] == 'randZRot_max':
            # random leaf rotating angle around z axis
            randZRot_max = float(words[1])
        elif words[0] == 'tortAng':
            tortAng = [float(c) for c in words[1:]]  # leaf twisting angle
        elif words[0] == 'tortAng_start_pos':
            # leaf twisting start position (relative to total leaf length)
            tortAng_start_pos = [float(c) for c in words[1:]]
        elif words[0] == 'tortAng_end_pos':
            # leaf twisting end position (relative to total leaf length)
            tortAng_end_pos = [float(c) for c in words[1:]]

        elif words[0] == 'stem_diameter':
            # *1.047 to ensure Perimeter of hexagon equals to circle (area ratio 91%)
            stem_diameter = np.array([float(c) for c in words[1:]])*1.047
        elif words[0] == 'stem_midH':
            # cm, >30 belongs to upper, <30 belongs to lower
            stem_midH = float(words[1])
            ...
            # convert project angle to plain angle
            LEMMA_ANG = math.atan(math.tan(LEMMA_ANG)*math.cos(SPIKELET_ANG))
        elif words[0] == 'Glume to mid-grain angle':
            glumeAng = float(words[1])*math.pi/180
            # convert project angle to plain angle
            glumeAng = math.atan(math.tan(glumeAng)*math.cos(SPIKELET_ANG))
        elif words[0] == 'Awn to rachis angle (face view)':
        ...
        elif words[0] == 'Awn middle thickness':
    ...
    for line in open(config_physiology_fn, 'r'):
        ...
        if words[0] == 'leaf_NC':
            ...
            leaf_SPAD_distribution = 9.70 * \
                leaf_LNC_distribution + 7.13  # LNC: %
            ...
            stem_LNC_distribution = np.array(
                [float(c) for c in words[1:]])  # upper, lower
    ...
    centerSpikeletH = 0.2  # default 0.2
    lemmaAng = LEMMA_ANG
    awnAng_coef_mid = awnAngCoef_Fitter(
        lemmaAng, spikeletAng, awnAng1, awnAng2)  # ,awnAng1,awnAng2
    awnAng_coef_bot = awnAngCoef_Fitter(
        lemmaAng, spikeletAng, awnAng1/2, awnAng2*2)  # awnAng1/2,awnAng2*2
    grainThick = (1+grainShrinkCoef)/2*grainThick
    grainLen = (1+grainShrinkCoef)/2*grainLen
    awnAng_coef_top = awnAngCoef_Fitter(
        lemmaAng, spikeletAng, awnAng1/4, awnAng2/4)  # awnAng1/4,awnAng2/4
    ...
    leaf_LNC_mean = np.mean(leaf_LNC_distribution[0:5]) # TODO Leaf LNC suppose have same no as leaves in a tiller, why take only the first 5 into account?
    ...
    plant_KP_imageCoor_dict, plant_KP_virtualCoor_dict = inputFileProcessing(
        dataIn, img_H, cultivarID, spikeInternodeLenZero, leafStraighten)

    Plants_Tillers_info = list(plant_KP_virtualCoor_dict.keys())
    Plants_Tillers_info.sort(key=lambda x: int(
        x.split('*')[1])*100+int(x.split('*')[2]))
    M_multi_tillers = []
    ...
    for block_i in Plants_Tillers_info:
        if 0:
            if not os.path.exists(tillerSkeletonDir):
                ...
                try:
                    ...
                    raise Exception(
                        'Cannot create tillerSkeletonDir directory: %s' % tillerSkeletonDir)
            fn_temp = block_i.replace('*', '-')
            fnOutPrefix = os.path.join(
                tillerSkeletonDir, 'TillerSkeleton_'+fn_temp)
            showOn = 0
            # plot tiller 2D skeleton for checking
            TillerSkeletonPlot(
                plant_KP_virtualCoor_dict[block_i], fnOutPrefix, showOn)
        ...
        stem_PTs = np.array(
            plant_KP_virtualCoor_dict[block_i]['stem'][0])*plantH_coef
        stem_PTs_num = len(stem_PTs)
        M_stem_all = np.zeros([0, 17])
        barcode = [plant_i, tiller_i, 0, 3, 0]
        stem_seg_len = [np.linalg.norm(np.array(
            stem_PTs[pt_i])-stem_PTs[pt_i-1]) for pt_i in range(1, stem_PTs_num-1)]
        ...
        for pt_i in range(0, stem_PTs_num-1):
            ...
            node_diameter_temp = np.dot(
                stem_diameter, coefs)/sum([coef1, coef2, coef3])
            stem_node_diameter.append(node_diameter_temp)
        # stem_PTs_num-1: do not use the last point (extendTip)
        for pt_i in range(1, stem_PTs_num-1):
            ave_height = (
                sum(stem_seg_len[0:pt_i-1])+sum(stem_seg_len[0:pt_i]))/2
            if ave_height > stem_midH*plantH_coef:
                LNC_Kt_Kr = [stem_LNC_distribution[0],
                             Kt_stem_distribution[0], Kr_stem_distribution[0]]
            else:
                LNC_Kt_Kr = [stem_LNC_distribution[1],
                             Kt_stem_distribution[1], Kr_stem_distribution[1]]
            organ_base = [stem_PTs[pt_i-1][0], 0, stem_PTs[pt_i-1][1]]
            organ_tip = [stem_PTs[pt_i][0], 0, stem_PTs[pt_i][1]]
            stem_seg_diameter = [
                stem_node_diameter[pt_i-1], stem_node_diameter[pt_i]]
            # plant, tiller, Position, organCode
            M_stem = buildStem(barcode, LNC_Kt_Kr, organ_base,
                               organ_tip, stem_seg_diameter)
            M_stem_all = np.vstack((M_stem_all, M_stem))
        ...
        for leaf_i in range(num_leaf):
            ...
            # ear presents, flag leaf
            if leaf_order == 1 and len(plant_KP_virtualCoor_dict[block_i]['ear'][0]) > 0:
            ...
            LNC_Kt_Kr = [leaf_LNC_distribution[leaf_order-1],
                         Kt_leaf_distribution[leaf_order-1], Kr_leaf_distribution[leaf_order-1]]
            leaf_PTs = np.array(
                plant_KP_virtualCoor_dict[block_i]['leaf'][leaf_i])
            leaf_base = np.array(leaf_PTs[0])*plantH_coef
            leaf_PTs = leaf_PTs-leaf_PTs[0]+leaf_base
            organ_PT_coords = [[leaf_PTs[i][0], 0, leaf_PTs[i][1]]
                               for i in range(len(leaf_PTs))]
            ...
            if abs(vec0_norm) < 0.0001:
                stem_seg_base = [stem_PTs[minDist_pos]
                                 [0], 0, stem_PTs[minDist_pos][1]]
                stem_seg_tip = [stem_PTs[minDist_post]
                                [0], 0, stem_PTs[minDist_post][1]]
            else:
                vec0 = vec0/vec0_norm
                ...
                if val1 < val2:  # at the same side of leaf line
                    ...
                    stem_seg_base = [stem_PTs[minDist_prev]
                                     [0], 0, stem_PTs[minDist_prev][1]]
                    stem_seg_tip = [stem_PTs[minDist_pos]
                                    [0], 0, stem_PTs[minDist_pos][1]]
            M_leaf = buildLeaf(barcode, LNC_Kt_Kr, organ_PT_coords, stem_seg_base, stem_seg_tip, widLen_relation_coef, widFunc, rollRad,
                               sheathRollRad, transLen, ptDense, halfSplit, randZRot_max, tortAng, tortAng_start_pos, tortAng_end_pos, leafAng_coef)
        ...
        if len(ear_PTs) and withSpike:
            ...
            LNC_Kt_Kr = [rachis_LNC, Kt_rachis, Kr_rachis, spikelet_LNC,
                         Kt_spikelet, Kr_spikelet, awn_LNC, Kt_awn, Kr_awn]
            organ_PT_coords = [[ear_PTs[i][0], 0, ear_PTs[i][1]]
                               for i in range(len(ear_PTs))]
            organ_PT_coords = np.array(organ_PT_coords)
            ear_PTs = np.array(ear_PTs)
            ear_len = sum([np.linalg.norm([ear_PTs[i]-ear_PTs[i+1]])
                          for i in range(len(ear_PTs)-1)])
            spikeletNum = round(float(ear_len*spikeletDense))
            M_ear = buildEar(barcode, LNC_Kt_Kr, organ_PT_coords, spikeletNum, spikeletBranchLen, spikeletBranchThick, awnLen, awnThick, grainLen, grainWid, grainThick, grainLen2, grainWid2,
                             grainThick2, grainShrinkCoef, awnAng_coef_bot, awnAng_coef_mid, awnAng_coef_top, glumeAng, LEMMA_ANG, SPIKELET_ANG, zRotAng, earRachis_diameter, allowedLayerNum, glumeAwnLen)
    ...
    if op.d3 == 1 or op.d3 == 3:  # if drawing single plant is turned on
        picOut_plant = fnOut_plant[0:-4]+'.png'
        command = 'python '+os.path.abspath(Path(__file__).parent)+'\plantDraw3D.py -i %s -o %s -ccs 4 -cql 0 -cqu 1 -cm plasma -alpha 1 -va1 0 -va2 -90' % (
            fnOut_plant, picOut_plant)
        try:
    ...
    M_canopy = buildCanopy(M_multi_tillers, row_dist, sowing_width,
                           plant_density, rowNum_simulate, rowLen_simulate, tillerAng, Kr_soil , groundH_mean, groundH_sd)
    print('  Writing Triangle-Patch into file...')
    format_str = '%d\t'*5+'%.3f\t'*12
    format_str = format_str[0:-1]
    np.savetxt(fnOut, M_canopy, fmt=format_str)  # , delimiter='\t'

    if op.d3 == 2 or op.d3 == 3:  # if drawing for canopy is turned on
        picOut_canopy = fnOut[0:-4]+'.png'
        command = 'python '+os.path.abspath(Path(__file__).parent)+'\plantDraw3D.py -i %s -o %s -ccs 4 -cql 0 -cqu 1 -cm plasma -alpha 1 -va1 30 -va2 30' % (
            fnOut, picOut_canopy)
        try:
            subprocess.run(command, check=True)
        except:
    ...
    print('  Canopy construction done! Time used: ', elapse)
